...
export class CircleCarousel {
    ...
    constructor(container, radius = () => window.innerWidth / 2, scaleMultiplier = 350) {
    ...
    drawElement(item?, angle?)
    {
        ...
        item.y = y;
        item.z =
            (item.y + this.radius) / (this.radius * 3) < 0
                ? 0
                : (item.y + this.radius) / (this.radius * 3);
        const opacity = Transitioner.EasingFunctions.easeInOutQuad(item.z)
        const _x = item.x
        const scale = item.z
    ...
    setSteps()
    {
        ...
        for(let i = 0, k = 0; i < orderedByZIndex.length; i++)
        {
            if(i === 0)
            {
                ...
                orderedByZIndex[i].element.style.transform = `translate(${orderedByZIndex[i].x}px,${Math.abs(orderedByZIndex[i].z*this.scaleMultiplier * 1.2)}px) scale(1)`;
                orderedByZIndex[i].element.style.opacity = `1`;
            }
            else
            {
                if(i%2===0)
